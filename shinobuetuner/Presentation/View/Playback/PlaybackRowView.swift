...
/// 音声一覧の1行を表示するビュー
struct PlaybackRowView: View {
    let playbackFile: PlaybackFile
    let isSelected: Bool
    let isPlaying: Bool
    ...
    var body: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                // ファイル名（拡張子を除く）
                Text(playbackFile.fileName.replacingOccurrences(of: ".m4a", with: ""))
    ...
    private var formattedDuration: String {
        let total = Int(playbackFile.duration)
    ...
    private var formattedSize: String {
        let kb = Double(playbackFile.fileSize) / 1024
        if kb < 1024 {
